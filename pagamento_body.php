...
?>
        <?php
            $sql = "
                ...
                FROM sm_pagamentos
                WHERE pag_data BETWEEN ADDDATE('".$_POST['ano']."-".$_POST['mes']."-01', INTERVAL -7 MONTH) AND '".$_POST['ano']."-".$_POST['mes']."-31'
...
$pesquisada = '2019-02-01';
$pesquisa = '2019-03-01';

/*
$date = new DateTime($pesquisada); // Data de Nascimento
$idade_acompanhamento = $date->diff(new DateTime($pesquisa)); // Data do Acompanhamento
...
$data_inicio = new DateTime($pesquisada);
$data_fim = new DateTime(date($pesquisa));
$dateInterval = $data_inicio->diff($data_fim);
echo $dateInterval->y."<br>";


echo dateDifference($pesquisada,$pesquisa, '%m')."<br>";


$data_inicial = $pesquisada;
$data_final = $pesquisa;
$diferenca = strtotime($data_final) - strtotime($data_inicial);
$meses = floor($diferenca / (60 * 60 * 24));
echo "A diferença é de $meses entre as datas<br>";


$date1=date_create($pesquisada);
$date2=date_create($pesquisa);
$diff=date_diff($date1,$date2);
echo $diff->format("%m")."<br>";
*/


$dataI = strtotime($pesquisada);
$dataF = strtotime($pesquisa);
$diferencaFI = $dataF - $dataI;
//echo date('m', $diferencaFI)."<br>";
            ...
            while ($dados = mysqli_fetch_array($consulta)) {
                ...
                for ($d=0;$d<count($infoPag);$d++) {
                    ...
                    //$diff = dateDifference($infoDados[0],$_POST['ano'].'-'.$_POST['mes'].'-31', '%m');

                    $dataI = strtotime($infoDados[0]);
                    $dataF = strtotime($_POST['ano'].'-'.$_POST['mes'].'-01');
                    $diferencaFI = $dataF - $dataI;

                    $diff = date('n',$diferencaFI);


                    $rep = $infoDados[1];
                    $mm = $infoDados[2];

                    //echo $dados[0]." | ".$infoDados[0]." | ".$infoDados[1]." | ".$infoDados[2]." | ".$diff."<br>";

                    switch ($rep) {
                        case 1: $mes[$diff]=$mm.$rep; break;
                        case 2: $mes[$diff]=$mes[($diff-1)]=$mm.$rep; break;
                        case 3: $mes[$diff]=$mes[($diff-1)]=$mes[($diff-2)]=$mm.$rep; break;
                        case 4: $mes[$diff]=$mes[($diff-1)]=$mes[($diff-2)]=$mes[($diff-3)]=$mm.$rep; break;
                        case 5: $mes[$diff]=$mes[($diff-1)]=$mes[($diff-2)]=$mes[($diff-3)]=$mes[($diff-4)]=$mm.$rep; break;
                        case 6: $mes[$diff]=$mes[($diff-1)]=$mes[($diff-2)]=$mes[($diff-3)]=$mes[($diff-4)]=$mes[($diff-5)]=$mm.$rep; break;
                    }
                }
                ...
                echo "<tr>
                        ...
                        <td align='center'>" . $mes[0]. "</td>
                    </tr>";

                //echo $mes[6]." | ".$mes[5]." | ".$mes[4]." | ".$mes[3]." | ".$mes[2]." | ".$mes[1]." | ".$mes[0]."<br>";
